...
class Program
{
    static void Main()
    {
        ...
        var accumulator = new MonadoBlade.Core.Concurrency.TaskBatchAccumulator(
        ...
        Console.WriteLine($"  Total Items Dispatched: {string.Join(", ", new List<int> { dispatchedBatches.Count > 0 ? dispatchedBatches[0].Count : 0 })}");
        ...
        var accumulator2 = new MonadoBlade.Core.Concurrency.TaskBatchAccumulator(_ => { }, 50, 10);
        ...
        accumulator2.Flush();
        System.Threading.Thread.Sleep(200);
        Console.WriteLine($"  Total Items Processed (100 expected): OK");
