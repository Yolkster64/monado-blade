"use strict";
/**
 * Circuit Breaker Pattern Implementation
 * Prevents cascading failures by monitoring provider health
 * States: Closed (normal) -> Open (failing) -> Half-Open (recovery test) -> Closed
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.CircuitBreaker = void 0;
const types_1 = require("./types");
class CircuitBreaker {
    constructor(providerId, config = {}) {
        this.providerId = providerId;
        this.state = types_1.CircuitBreakerState.CLOSED;
        this.failureCount = 0;
        this.successCount = 0;
        this.lastFailureTime = 0;
        this.nextRetryTime = 0;
        this.backoffMultiplier = 1;
        this.stateChangeListeners = [];
        this.config = {
            failureThreshold: config.failureThreshold ?? 5,
            successThreshold: config.successThreshold ?? 3,
            timeout: config.timeout ?? 60000,
            initialBackoff: config.initialBackoff ?? 1000,
            maxBackoff: config.maxBackoff ?? 60000
        };
    }
    /**
     * Record a health check result and update circuit breaker state
     */
    recordResult(result) {
        if (result.status === 'HEALTHY') {
            this.recordSuccess();
        }
        else {
            this.recordFailure(result.error || 'Unknown error');
        }
    }
    /**
     * Record a successful check
     */
    recordSuccess() {
        this.failureCount = 0;
        if (this.state === types_1.CircuitBreakerState.HALF_OPEN) {
            this.successCount++;
            if (this.successCount >= this.config.successThreshold) {
                this.transitionTo(types_1.CircuitBreakerState.CLOSED, 'Recovery successful');
                this.successCount = 0;
                this.backoffMultiplier = 1;
            }
        }
        else if (this.state === types_1.CircuitBreakerState.CLOSED) {
            this.successCount = 0;
        }
    }
    /**
     * Record a failed check
     */
    recordFailure(reason) {
        this.lastFailureTime = Date.now();
        this.failureCount++;
        this.successCount = 0;
        if (this.state === types_1.CircuitBreakerState.HALF_OPEN) {
            this.transitionTo(types_1.CircuitBreakerState.OPEN, `Recovery attempt failed: ${reason}`);
            this.scheduleNextRetry();
        }
        else if (this.state === types_1.CircuitBreakerState.CLOSED &&
            this.failureCount >= this.config.failureThreshold) {
            this.transitionTo(types_1.CircuitBreakerState.OPEN, `Failure threshold exceeded: ${reason}`);
            this.scheduleNextRetry();
        }
    }
    /**
     * Check if the circuit breaker allows requests
     */
    canExecute() {
        if (this.state === types_1.CircuitBreakerState.CLOSED) {
            return true;
        }
        if (this.state === types_1.CircuitBreakerState.OPEN) {
            if (Date.now() >= this.nextRetryTime) {
                this.transitionTo(types_1.CircuitBreakerState.HALF_OPEN, 'Attempting recovery');
                return true;
            }
            return false;
        }
        // HALF_OPEN: allow request to test recovery
        return true;
    }
    /**
     * Schedule the next retry attempt with exponential backoff
     */
    scheduleNextRetry() {
        const backoff = Math.min(this.config.initialBackoff * this.backoffMultiplier, this.config.maxBackoff);
        this.nextRetryTime = Date.now() + backoff;
        this.backoffMultiplier = Math.min(this.backoffMultiplier * 2, 32);
    }
    /**
     * Transition to a new circuit breaker state
     */
    transitionTo(newState, reason) {
        if (newState === this.state) {
            return;
        }
        const change = {
            providerId: this.providerId,
            oldState: this.state,
            newState,
            timestamp: Date.now(),
            reason
        };
        this.state = newState;
        this.notifyStateChange(change);
    }
    /**
     * Get current state
     */
    getState() {
        return this.state;
    }
    /**
     * Reset the circuit breaker
     */
    reset() {
        this.transitionTo(types_1.CircuitBreakerState.CLOSED, 'Manual reset');
        this.failureCount = 0;
        this.successCount = 0;
        this.backoffMultiplier = 1;
        this.nextRetryTime = 0;
    }
    /**
     * Get remaining time until next retry (in milliseconds)
     */
    getTimeUntilRetry() {
        if (this.state === types_1.CircuitBreakerState.CLOSED) {
            return 0;
        }
        return Math.max(0, this.nextRetryTime - Date.now());
    }
    /**
     * Subscribe to state change events
     */
    onStateChange(listener) {
        this.stateChangeListeners.push(listener);
    }
    /**
     * Notify all listeners of state change
     */
    notifyStateChange(change) {
        for (const listener of this.stateChangeListeners) {
            listener(change);
        }
    }
    /**
     * Get detailed state information
     */
    getDetailedState() {
        return {
            state: this.state,
            failureCount: this.failureCount,
            successCount: this.successCount,
            lastFailureTime: this.lastFailureTime,
            nextRetryTime: this.nextRetryTime,
            timeUntilRetry: this.getTimeUntilRetry(),
            backoffMultiplier: this.backoffMultiplier
        };
    }
}
exports.CircuitBreaker = CircuitBreaker;
//# sourceMappingURL=CircuitBreaker.js.map